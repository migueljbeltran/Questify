    ...
    if (chores.length === 0) {
        return (
            <div className="text-center py-12 border border-dashed border-slate-800 rounded-2xl">
                <p className="text-slate-500">No active quests. Create one to get started!</p>
            </div>
        )
    }
                ...
                >
                    <div className="flex-1 min-w-0 mr-4">
                        <div className="flex items-center gap-3 mb-1">
                            <h4 className="font-medium text-slate-200 truncate">{chore.title}</h4>
                            <span className="px-2 py-0.5 rounded-full bg-slate-800 text-[10px] font-medium text-sky-400 border border-slate-700">
                                +{chore.base_xp} XP
                            </span>
                        </div>
                        {chore.description && (
                            <p className="text-xs text-slate-500 truncate">{chore.description}</p>
                        )}
                    </div>
